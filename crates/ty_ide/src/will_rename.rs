...
//! Directory renames are limited to regular packages that keep the same logical parent;
//! namespace and cross-parent package moves are ignored.
//!
...
/// including moved sources, and the caller must enable place-load recording before creating its
/// database snapshot. `in_scope` restricts both the rename sources and the files that can be edited.
///
        ...
        let (old_name, package) = match rename.kind {
            ...
            RenameKind::Directory => {
                ...
                let name = inits
                    ...
                    .name(db)
                    .clone();
                // Typing and runtime resolution must both refer to this directory. Otherwise a
                // package with the same name elsewhere would be included in the rename.
                if [
