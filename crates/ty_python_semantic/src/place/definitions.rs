...
/// unbound or deleted, and whether lookup crosses a `global` or `nonlocal` declaration.
...
impl<'db> DefinitionResolution<'db> {
    ...
    fn from_builtin(
        ...
        if builtins_scope == scope {
            ...
            // that occurs after this load. Retain the value's boundness, but omit that later
            // definition and mark the resolution incomplete if a value was found.
...
impl<'db> DefinitionResolutionBuilder<'db> {
    ...
    /// Finishes the record using inference's final boundness and scope-declaration state.
    pub(crate) fn finish(
        mut self,
        is_definitely_bound: bool,
        crosses_scope_declaration: bool,
    ) -> DefinitionResolution<'db> {
        // Individual sources can be unbound while a later fallback supplies a value. Inference
        // already combines those sources, so its final boundness is authoritative.
