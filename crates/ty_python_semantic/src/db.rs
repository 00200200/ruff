...
/// Sets whether the database can record place loads, initially leaving recording off for every file.
///
/// Call once when constructing a database, before running inference or creating snapshots.
/// The policy cannot change during the database's lifetime. With [`PlaceLoadRecordingMode::OnDemand`],
/// callers can select files later through [`enable_place_load_recording`].
///
/// The input must exist before inference runs: Salsa does not track the absence of a singleton,
/// so creating it when recording is first requested would not invalidate earlier inference.
...
/// Enables recording of place loads the next time the selected files are inferred.
///
/// This does not run inference. The selection persists across requests and source edits.
/// Selecting a new file invalidates inference that ran without recording; selecting an already
/// enabled file does not change the database. This has no effect when the database was constructed
/// with [`PlaceLoadRecordingMode::Disabled`].
...
/// Returns whether `file` is selected, keeping the dependency on the file set behind a query.
///
/// When another file is selected, this query's result stays the same. Salsa can then reuse
/// inference that depends on this result instead of rerunning it for every change to the set.
