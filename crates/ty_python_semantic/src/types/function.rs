...
use crate::types::diagnostic::{
    ASSERT_TYPE_UNSPELLABLE_SUBTYPE, DISJOINT_CAST, DISJOINT_CAST_STRICT, INVALID_ARGUMENT_TYPE,
            ...
            KnownFunction::Cast => {
                    ...
                    && !source_type.is_equivalent_to(db, env, Type::Never)
                {
                    let code = if source_type
                        .discard_specialization(db, env)
                        .is_disjoint_from(db, env, casted_type.discard_specialization(db, env))
                    {
                        &DISJOINT_CAST
                    } else {
                    ...
                    if let Some(builder) = context.report_lint(code, call_expression) {
