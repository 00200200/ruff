            ...
            let ty = match choose(path_bound.variance(), path_bound) {
                ...
                PathBoundSolution::ViolatesDeclaredUpperBound => {
                    violations.push(SolutionViolation {
                        ...
                        kind: SolutionViolationKind::UpperBound(path_bound.evidence_lower()),
                    });
                    None
                }
        ...
        match bound_typevar.require_bound_or_constraints(db, env) {
            ...
            TypeVarBoundOrConstraints::Constraints(constraints) => {
                ...
                let Some(compatible_constraint) = compatible_constraint else {
                    ...
                    {
                        return PathBoundSolution::ViolatesDeclaredConstraints(
                            ConstraintFailureEvidence::Upper(
                                path_bound.upper.iter_evidence().collect(),
                            ),
                        );
                    }
    ...
    fn solutions_distinguish_declaration_failures_from_conflicting_bounds() {
        ...
        for (typevar, lower, upper, expected_violation) in [
            ...
            (
                ...
                Some((
                    TypeVarVariance::Invariant,
                    SolutionViolationKind::UpperBound(Some(str)),
                )),
            ),
        ] {
