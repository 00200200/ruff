...
impl<'db> SolutionWalker<'db> {
    ...
    fn validate_constrained_and_then<L: SolutionLimits>(
        ...
        let mut evidence = CandidateTypeVarRangeSolutionBuilder::default();
        for (constraint, _) in path.positive_constraints() {
            ...
            evidence.add_constraint(db, env, bound_typevar, constraint);
        }
        let Some(evidence) = evidence.finish(db, env, storage) else {
        ...
        let has_non_concrete_evidence = has_no_evidence
            || evidence.has_only_gradual_evidence
