...
impl Session {
    ...
    /// Enables a background request's recording selection if its session revision is still current.
    ///
    /// A stale selection has no effect. The caller must retry the request so it can discover files
    /// again against the current projects before taking a snapshot with recording enabled.
    pub(crate) fn enable_place_load_recording(
        ...
        // Do not apply its file handles to the current databases.
