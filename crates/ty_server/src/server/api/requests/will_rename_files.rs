//! Returns import and reference edits before the client renames Python files or regular packages.
//!
//! Each rename is analyzed in the workspace that owns both paths. Candidates include indexed and
//! open files plus Python files discovered on disk inside a renamed folder. If a candidate needs
//! name-load recording, the main thread enables it and retries the request with a fresh snapshot.
//!
...
/// Returns candidate files that still need recording enabled, grouped by their snapshot project index.
///
/// This discovers files without inferring types. The main thread enables recording before retrying
/// the request with a fresh snapshot. Files already selected by earlier requests need no preparation.
...
/// Validates workspace ownership and discovers the Python files moved by one request entry.
///
/// Returns `Ok(None)` for non-Python files or directories without Python sources. An error records
...
/// Finds Python files on disk, returning `None` if any directory entry cannot be read.
///
/// This deliberately bypasses project exclusions because the filesystem move includes excluded
/// files too. Traversal does not descend into entries that are directory symlinks.
...
/// Recognizes an explicit non-Python extension even when the URI is not a valid local file path.
